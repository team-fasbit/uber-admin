...
if (!defined('CallCenterManager')) define('CallCenterManager',1);
...
class ManagerApiController extends Controller
{
    ...
    {
        $this->middleware('ManagerApiVal' , array('except' => ['register' , 'login' , 'send_otp' , 'forgot_password']));
    ...
    public function payment_mode_update(Request $request) {
        ...
        $validator = Validator::make($request->all() ,
            array(
                'payment_mode' => 'required|in:'.COD.','.PAYPAL.','.CARD.','.WALLETBAY,
                )
            );
         if($validator->fails()) {
            $error_messages = implode(',', $validator->messages()->all());
    ...
    public function validate_promo(Request $request)
    {
        ...
        else 
        {
            if($promo_code !='') 
            {
                $getPromo = PromoCode::where('coupon_code', $promo_code)->where('scope', LIMITED_OFFER)->whereRaw('"'.$current.'" between `start` and `end`')->first();
                if($getPromo) {
                    $response_array = array('success' => true, 'error_message' => 'Valid Promo Code');
                }
                else {
            ...
            {
                $response_array = array('success' => true, 'error_message' => 'No Promo Code Applied');
            }
        }
        $response = response()->json($response_array, 200);
        return $response;
