    ...
    public function handle($request, Closure $next)
    {
        ...
        $validator = Validator::make(
                ...
                array(
                        ...
                        'id' => 'required|integer|exists:call_center_managers,id'
                ));
        ...
        if ($validator->fails()) {
            ...
            if (!Helper::is_token_valid('CallCenterManager', $user_id, $token, $error)) {

                $response = response()->json($error, 200);
                return $response;
            } else {
                $user = CallCenterManager::find($request->id);
