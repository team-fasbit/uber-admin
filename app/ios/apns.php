...
define("User_Certi_Path", 'user.pem');

define("Provider_Certi_Path", 'driver.pem');

define("PassPH", 'nikola123');

class Apns {

    private $passphrase = PassPH;
    private $certificateUser = User_Certi_Path;
    private $certificateProvider = Provider_Certi_Path;
    private $defa = DefA;
    ...
    private function getCertificatePath($user_type) {
        if($user_type == 1) //user
        {
            Log::info('path user = '.print_r(app_path().'/ios/iph_cert/'.$this->certificateUser,true));
            return app_path().'/ios/iph_cert/'.$this->certificateUser;
        }
        else
        {
            Log::info('path provider = '.print_r(app_path().'/ios/iph_cert/'.$this->certificateProvider,true));
            return app_path().'/ios/iph_cert/'.$this->certificateProvider;
        }
    }

    public function __construct($user_type) {
        $this->initialize_apns($user_type);
    }

    public function initialize_apns($user_type) {
        try {
            $this->ctx = stream_context_create();

            //stream_context_set_option($ctx, 'ssl', 'cafile', 'entrust_2048_ca.cer');
            stream_context_set_option($this->ctx, 'ssl', 'local_cert', $this->getCertificatePath($user_type));
